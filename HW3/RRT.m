...
classdef RRT < Navigation
    ...
    function p = path(rrt, xstart, xgoal)
        ...
        grid
        xyzlabel
        [x y theta] = goToPose(xstart, g.coord(vstart))
        ...
        for k= p(end-1:-1:1)
            ...
            if ~isempty(b)
                ...
                % add the following two lines for HW3, plot
                vel = [vel b.vel*ones(1, b.k)];
                steer = [steer b.steer*ones(1, b.k)];
            else
                keyboard; 
            end
        end
        % add for ploting steering and velocity profile
        t=0:0.15:(size(vel,2)-1)*0.15;
        figure;plot(t,vel);
        figure;plot(t,steer);
        figure;
        %keyboard;
        ...
        % add local planner
        [x y theta] = goToPose(g.coord(vgoal), xgoal)
        path = [path [x;y;theta]];
        % end local pl
        clf
        plot2(path');
...
%                 ntrials = 50;
                ...
                %               add the following code for HW3
                % plot(best.path(1,1:best.k), best.path(2,1:best.k), '.');
                
                %% best.path should be tranposed before being fed into clearpath
                if ~rrt.clearpath(best.path(1:2,1:best.k)')
                    %disp('path collision');
                    continue;
                end
                %               % End  
                
                % HW3  set the angle to be in the range of [-pi, pi]
...
function x = pathsim(x0, u)
    ...
    % comment for HW3
    % tmax = 1;
    % experiments with integration time
    tmax =5;
    %end
...
% HW3 experiments with max steering angle
steermax = 2.4;
% speed = 1;
% HW3 experiments with max steering angle
speed = 1;
